...
type Decoder struct {
	...
	// metaLocker guards SampleRate/Channels/Kbps/Layer, written by the
	// decoder goroutine and read by Read and by callers after the first
	// frame. (Upstream wrote these without a lock; fixed for -race.)
	...
	// originalEof is set by the reader goroutine and read by Read; atomic to
	// avoid the upstream unlocked bool race.
	originalEof atomic.Bool
}
...
func (dec *Decoder) Started() (channel chan bool) {
	...
	go func() {
		for {
			...
			dec.decoderLocker.Lock()
			decodedLen := len(dec.decodedData)
			dec.decoderLocker.Unlock()
			if decodedLen != 0 {
				channel <- true
			} else {
...
// Meta returns the stream metadata (sample rate, channels, bitrate, layer)
// under the metadata lock. It is safe to call from any goroutine once the
// decoder has decoded at least one frame.
func (dec *Decoder) Meta() (sampleRate, channels, kbps, layer int) {
	dec.metaLocker.Lock()
	defer dec.metaLocker.Unlock()
	return dec.SampleRate, dec.Channels, dec.Kbps, dec.Layer
}
...
func (dec *Decoder) Read(data []byte) (n int, err error) {
	for {
		...
		dec.readerLocker.Lock()
		dataLen := len(dec.data)
		dec.readerLocker.Unlock()
		dec.decoderLocker.Lock()
		decodedLen := len(dec.decodedData)
		dec.decoderLocker.Unlock()
		if dataLen == 0 && decodedLen == 0 && dec.originalEof.Load() {
			err = io.EOF
			return
