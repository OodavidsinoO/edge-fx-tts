...
func TestDecodeSampleStream(t *testing.T) {
	...
	// so the observed count is slightly lower and jitters (173-177 frames =
	// 99648-101952). The bound catches channel-width regressions: consuming a
	// mono stream as stereo halves the count to ~52k, far below this floor.
	if total < 97500 {
		t.Fatalf("decoded %d frames, want >= 97500 (mono stream; 2x regression would yield ~52k)", total)
	}
