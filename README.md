...
- **11 built-in preset profiles** — filmai ×2 (D3–D4 movie-AI), broadcast ×3
  (announcer/radio/teleconference), aifake / doubledelay / scifiatmo (A/B/C
  demo presets), jarvis / edith / ai-modern (modern film-AI voices), plus a
...
| `doubledelay` | Cinematic double + slapback (report §3.2 B) | HPF 80 Hz; comp 3:1 / −18 dB; chorus 25 ms ×2 wet 0.35; slapback 100 ms, zero feedback; LPF 7 kHz + wideband de-esser (7 kHz / −28 dB); FDN RT60 1.6 s |
| `scifiatmo` | Sci-fi atmosphere (report §3.2 C) | HPF 80 Hz; comp 4:1 / −16 dB; wide chorus 30 ms ×3; ambient delay 250 ms, FB 0.25; LPF 7 kHz + wideband de-esser (7 kHz / −28 dB); FDN RT60 3.0 s damp 0.38 |
| `filmai-d3` | D3 HAL/TARS calm server voice | WSOLA −2.5 st; gate −45 dB 10:1; LPF 7 kHz; comp 4:1 fast attack; FDN RT60 0.25 s wet 0.1; wideband de-esser (7 kHz / −28 dB) + LPF 7 kHz |
...
- The removed `filmai` / `filmai-d2` / `broadcast-e1` were duplicate aliases
  of `jarvis` / `ai-modern` / `broadcast` (chains byte-identical, apart from
  a comment header on `broadcast-e1`); use the canonical names since v0.5.5.
